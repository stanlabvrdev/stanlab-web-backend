...
it("should update the currently selected school", async () => {
  ...
  expect(updatedProfile.selectedSchool.toString()).toBe(schoolTeacher.school.toString());
});

it("should set the currently selected school to null", async () => {
  const teacher = await global.loginTeacher();
  ...
  expect(res.statusCode).toBe(200);
  expect(updatedProfile).toBeDefined();
  expect(updatedProfile.selectedSchool).toBe(null);
});
